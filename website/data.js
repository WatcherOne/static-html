// desc 可能用不到，在详细里面会具体说的

const urlList = [
    {
        id: 1,
        title: 'link-to-qr',
        url: 'https://link-to-qr.com',
        introduction: '在线制作二维码',
        detailUrl: 'detail/1.html',
        desc: '可以调整二维码颜色, 大小, 文案等。windows 与 Mac 系统上表现不一样'
    },
    {
        id: 2,
        title: 'Method Draw Vector Editor',
        url: 'https://editor.method.ac',
        introduction: '在线制作svg图',
        detailUrl: 'detail/2.html',
        desc: '可以导入图片, 或者选择一些形状或自带的物体'
    },
    {
        id: 3,
        title: 'Print.js',
        url: 'https://printjs.crabbly.com',
        introduction: '打印pdf插件',
        detailUrl: 'detail/3.html',
        desc: '可以下载服务器pdf, 可以将html文档转成pdf下载, 图片, json数据, 未使用过'
    },
    {
        id: 4,
        title: 'SMS-Activate',
        url: 'https://sms-activate.org',
        introduction: '外网虚拟号码服务平台',
        detailUrl: 'detail/4.html',
        desc: '接受短信验证码, (可以租4h ~ 4周), 选择国家、项目',
        supplement: '账号: 286154864@qq.com, 密码大小写; 剩余价值: 91.36₽; 请联系作者使用 或 自己注册并充值'
    },
    {
        id: 5,
        title: 'Byrutor',
        url: 'https://byrutdb.org',
        introduction: '盗版游戏下载A-Byrutor',
        keywords: '毛子,破解游戏',
        detailUrl: 'detail/5.html',
        desc: '一款来自俄罗斯的破解游戏资源网站, 基本是全Steam搬过来了, 所有资源以BT种子的形式保存, 可以直接使用迅雷下载. 网站语言为俄文, 目前网站完全开放, 国内可直接访问'
    },
    {
        id: 6,
        title: 'RuTracker',
        url: 'https://rutracker.net',
        introduction: '盗版游戏下载B-RuTracker',
        keywords: '毛子,盗版游戏',
        detailUrl: 'detail/6.html',
        desc: 'RuTracker是俄罗斯此前最大的的盗版资源下载网站, 提供了大量热门软件、影视、音乐、动漫、游戏的免费破解资源;在国内流传的许多绿色汉化版软件或游戏, 都是在RuTracker的基础上加上汉化补丁的, 需要注册账号, 需要浏览器翻译插件',
    },
    {
        id: 7,
        title: 'Switch520',
        url: 'https://download.fourpetal.com',
        introduction: '盗版游戏下载C-Switch520',
        keywords: 'switch',
        detailUrl: 'detail/7.html',
        desc: 'switch520.com原域名已改. 国内游戏下载网址, 服务器在香港, 可以直接下载, 广告太多了'
    },
    {
        title: 'torrents',
        url: 'https://rarbgprx.org/torrents.php',
        introduction: '2K~4K视频源',
        keywords: 'vpn',
        desc: '需要翻墙'
    },
    {
        title: 'OpenAI',
        url: 'https://openai.com',
        introduction: 'openAI-官网',
        keywords: 'vpn?',
        desc: 'openAI公司官网'
    },
    {
        title: 'openAI.chat',
        url: 'https://chat.openai.com',
        introduction: 'openAI-chat',
        keywords: 'vpn!',
        desc: '提供chatGPT产品服务, 使用平台, 需要 VPN, 翻墙了也不一定能访问到，专属的vpn线路',
        supplement: '账号：zhubo.watcher@gmail.com[谷歌账号], 密码大小写'
    },
    {
        title: 'openAI.doc',
        url: 'https://platform.openai.com',
        introduction: 'openAI-doc',
        desc: '提供用户登录API文档说明, 常用示例等功能'
    },
    {
        title: 'openAI.api',
        url: 'https://api.openai.com',
        introduction: 'openAI-api',
        desc: '提供后端接口的服务器, 此url就是一个api接口'
    }
]
